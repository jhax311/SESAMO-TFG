...
class hojasPre extends conexion
{
    private $tabla = "hoja_prescripcion";
    ...
    private $dosis = null;
    private $via_administracion = null;
    private $frecuencia = null;
    private $fecha_fin_tratamiento = null;
    private $nhc = null;
    ...
    public function obtenerHojaNhc($campo, $valor)
    {
        if ($campo == 0) {
            $campo = "nhc";
        } else {
        ...
        if (count($datos) > 0) {
            ...
            $_respuestas = new respuestas;
            return $_respuestas->error_200("No existen hojas de prescripción.");
        }
    }
    ...
    public function post($json) //reciben un json como parametro
    {
        ...
        if (!isset($datos["token"])) {
            ...
            if ($arrayToken) {
                ...
                if (
                    !isset($datos["especialidad"]) || !isset($datos["nhc"]) || !isset($datos["principio_activo"])
                    || !isset($datos["dosis"]) || !isset($datos["via_administracion"]) || !isset($datos["frecuencia"]) || !isset($datos["fecha_fin_tratamiento"])
                ) {
                    ...
                    if (isset($res["result"])) {
                        ...
                        $respuesta["result"] = array(
                            "msg" => "Se ha creado la hoja de prescripción."
                        );
                        return $respuesta;
                    } else {
                ...
                return $_respuestas->error_401("El token es invalido o caducadooooo");
            }
        }
    }
    ...
    private function insertarHoja()
    {
        ...
        if (!$this->comprobarPaciente($this->nhc)) {
            ...
            $query = "INSERT INTO $this->tabla (especialidad, principio_activo, dosis, via_administracion, frecuencia, fecha_fin_tratamiento, nhc ) ";
            $query .= " VALUES ( ?, ?, ?, ?, ?, ?, ?)";
