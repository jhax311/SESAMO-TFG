...
require_once "clases/alta.class.php";
include 'config.php';
//inicializamos
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    switch ($partes[3]) {
        ...
        default:
            if (is_numeric($partes[3])) {//comprobamos que sea un numero
                ...
                $hojaP = $_alta->obtenerAltaNhc(0, $nhc); 
                ...
                if (isset($hojaP["result"])) {
                    ...
                    http_response_code(200);
                    echo json_encode($_respuestas->error_404("No hay altas asociadas al paciente."));
                }
            } else {
                //si no es ninguna de esas error
                header("Content-Type: application/json");
    ...
    //enviamos todo al controlador, clas alta, al metodo de post
    $datosArray = $_alta->post($postBody);
    header("Content-Type: application/json");
